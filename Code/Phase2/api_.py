...
from Code.Phase2.predictor import PosPrediction
...
class PRN_:
    ...
    edit for 2 faces referred from : https://github.com/akathpal/UMD-CMSC733-ComputerVision/tree/master/FaceSwap
    '''
    def __init__(self, is_dlib = False, prefix = '.'):
        ...
        if is_dlib:
            import dlib
            detector_path = os.path.join(prefix, './Code/Phase2/Data/net-data/mmod_human_face_detector.dat')
            self.face_detector = dlib.cnn_face_detection_model_v1(
                    detector_path)

        #---- load PRN 
        self.pos_predictor = PosPrediction(self.resolution_inp, self.resolution_op)
        prn_path = os.path.join(prefix, './Code/Phase2/Data/net-data/256_256_resfcn256_weight')
        ...
        # uv file
        self.uv_kpt_ind = np.loadtxt(prefix + '/Code/Phase2/Data/uv-data/uv_kpt_ind.txt').astype(np.int32) # 2 x 68 get kpt
        self.face_ind = np.loadtxt(prefix + '/Code/Phase2/Data/uv-data/face_ind.txt').astype(np.int32) # get valid vertices in the pos map
        self.triangles = np.loadtxt(prefix + '/Code/Phase2/Data/uv-data/triangles.txt').astype(np.int32) # ntri x 3
